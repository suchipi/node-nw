...
module.exports = function setupPipeWrenchSockets(target) {
  ...
  const cleanupStdout = pipeWrench.server(
    identifiers.stdout,
    function (socket) {
      socket.pipe(process.stdout);
      onExit(0, function closeStdoutSocket() {
  ...
  const cleanupStderr = pipeWrench.server(
    identifiers.stderr,
    function (socket) {
      socket.pipe(process.stderr);
      onExit(0, function closeStderrSocket() {
  ...
  const cleanupStdin = pipeWrench.server(identifiers.stdin, function (socket) {
    ...
    onExit(0, function closeStdinSocket() {
  ...
  const cleanupIpc = pipeWrench.server(identifiers.ipc, function (socket) {
    ...
    onExit(0, function closeIpcSocket() {
  ...
  const cleanupRepl = pipeWrench.server(identifiers.repl, function (socket) {
    ...
    onExit(0, function closeReplSocket() {
  ...
  onExit(0, function cleanUpPipeWrenchSockets() {
